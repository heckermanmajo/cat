...
function setupDB(): void {
    Model::updateTable(License::class);
    Model::updateTable(Activation::class);
    Model::updateTable(AuditLog::class);
}
...
body { font-family: system-ui, sans-serif; font-size: 14px; line-height: 1.5; padding: 20px; max-width: 1000px; margin: 0 auto; }
...
nav a { margin-right: 20px; text-decoration: none; padding: 5px 0; }
nav a:hover, nav a.active { border-bottom: 2px solid #000; }
.card { border: 1px solid #000; margin: 15px 0; }
.card-header { background: #000; color: #fff; padding: 8px 12px; font-weight: 600; }
...
input, select, textarea { width: 100%; padding: 8px; border: 1px solid #000; font-size: 14px; }
input:focus, select:focus, textarea:focus { outline: 2px solid #000; outline-offset: 1px; }
input[type="checkbox"] { width: auto; margin-right: 8px; }
button, .btn { display: inline-block; padding: 8px 16px; background: #000; color: #fff; border: 1px solid #000; cursor: pointer; text-decoration: none; font-size: 14px; }
...
.badge-warn { background: #ffd; color: #660; }
.badge-err { background: #fdd; color: #600; }
.alert { padding: 10px; margin: 10px 0; border: 1px solid; }
.alert-ok { background: #dfd; border-color: #0a0; }
.alert-err { background: #fdd; border-color: #c00; }
.stats { display: flex; gap: 15px; margin: 15px 0; }
.stat { border: 1px solid #000; padding: 15px; text-align: center; flex: 1; }
...
function nav(): void {
    ...
    $links = ['dashboard' => 'Dashboard', 'licenses' => 'Lizenzen', 'activations' => 'Aktivierungen', 'audit' => 'Audit Log'];
    echo '<nav>';
    foreach($links as $file => $label) {
        $active = $current === $file ? ' class="active"' : '';
        echo "<a href=\"$file.php\"$active>$label</a>";
    }
    echo '<a href="logout.php" style="float:right">Abmelden</a></nav>';
}

function head(string $title): void {
    ?><!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width">
    <title><?= $title ?> - CatKnows</title><style><?= css() ?></style></head><body>
    <h1>CatKnows Admin</h1><?php nav();
}
